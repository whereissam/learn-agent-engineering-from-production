...
**Prod 篇不進閱讀順序**，因為它不是「下一步」，是「另一個階段」。
...
> Hermes 是長期運作的 personal agent platform；
...
對應到層次框架。**原本寫「四層」，現在是五層**：
「長期運作」本來被塞在 Harness 裡，但排程和委派跟權限、沙箱不是同一類問題，
...
> ⚠ **已被後續取代。** 下面那個假回報的比率在重測時沒有維持住
> （見 2026-08-01 大掃除那一節）。留著是因為當時相信的東西也是紀錄的一部分，
> 而且重測的意義只有放在它旁邊才讀得出來。
...
- 實測記錄（真的 Gemini 3.6 Flash）：
  ...
  2. 然後測了 fake provider 測不到的那條路徑：把 server 殺掉、
...
2. **抓到一個更嚴重的**：3/3 把「8/1」填成 **2024**-08-01
...
> ⚠ **已被後續取代。** 這個載荷不再打得到這個比率，而原因也不是當初記下的那個——
> 見 2026-08-02 收尾那一節。留著是因為中間那個錯誤的診斷才是有教育性的部分。
...
#### Lesson 16 補課：Hermes 那句「never routes」講得太滿
...
**盤點的行數要更正**：`cron/` 是 **9 個檔、8727 行**（實際 `wc -l`），
原本寫 11 檔 8954 行。
...
> ⚠ **已被後續取代。** 這幾次只跑了 reload 那個題目，而在那個題目上守衛不動才是對的——
> 所以它們對守衛的 recall 完全沒有量到。見 2026-08-02 收尾那一節。
> 留著是因為把那個誤讀成及格，正是最值得記住的失敗。
...
**這跟 Lesson 8 的結果相反**（那邊被拒絕後連換五種工具繞道、最後謊報完成），
而最明顯的差異在拒絕訊息：
...
> ⚠ **已被後續取代。** 下面那個成本倍數後來又量了兩次、兩次都動了；
> 但書那個結果則反過來了。見 2026-08-02 收尾那一節。
> 留著是因為這一課現在改成以「哪一個重測結果一樣」開場。
...
## Lesson 17 補課：排序壞掉之後，下游 agent 會怎樣
...
> ⚠ **已被後續取代。**「不需要補」對 fixture 成立，對檢查器不成立：
> 一個連字號的 bug、以及一個被讀錯的假陽性率，後來都被找出來了。
> 見 2026-08-02 收尾那一節。
...
> 這三條的號碼在 2026-07-30 各往後挪了一號，因為原則 8 被
...
- ~~下面列的開源專案還沒逐一讀過原始碼~~ → **Lesson 23 讀完了**：
...
- 最重要的一段：靜默的抽取失敗。問「waist_yaw 在 2026 SDK 是第幾號」
  （答案在一個 HTML `<table>` 裡）：
...
- 檢索變好，agent 沒有變好。同一個問題（Lesson 20 那題）跑兩次，
...
- `total_tokens` 遠大於 `prompt + completion`（Gemini 3.6 Flash）：
...
- **第五次「沒出錯、只是沒資料」**：`shared/streaming/openai.ts` 的
...
- **為什麼要加第四個專案：Pi / OpenWorker / Hermes 都是應用**，
...
- 這課違反設計原則 6，而且跟 Lesson 24 是同一種違反：
...
**這一課真正要教的那個窗口**（一開始想成「有 journal 就安全」是錯的）：
...
**兩個設計細節要照抄，因為方向是相反的**（README「Dual Isolation Model」）：
...
**實作時差點做出一個假陽性製造機**：`ToolRecord` 一開始沒有
...
順帶抓到 Lesson 2 那個沙箱逃逸的第二次發生，隔了 27 課。
...
**中斷從來沒發生** —— delta 粒度不是你能控制的，又一個假陰性。
...
- **要做的 fault injection 矩陣**（在不同位置中斷，重啟後檢查）：
  ...
  每一格的失敗都是**安靜的**（設計原則 7）：永久 pending 的工具、
...
### 先講一件盤點時才發現的事：這個 repo 已經不是原本那個了
...
**這一段原本寫成 `All-Hands-AI/OpenHands → OpenHands/agent-canvas`，
...
**第一版會給出方向相反的結論**：只記 `risk`、沒填就算 UNKNOWN，
...
- `observation-event.ts:9` 那個 `source: "environment"` 是整課的重點。
...
- **四個我們沒有、但一看就知道為什麼要有的欄位**：
  ...
  | 欄位 | 位置 | 它解決了我們踩過的哪個坑 |
  |---|---|---|
  | `llm_response_id`（把同一次回應發出的平行工具呼叫綁在一起） | `action-event.ts:57` | **Lesson 23 那個潛伏三課的 bug：Gemini 不送 `index`，平行工具呼叫被串成一個壞字串。我們是靠 `index ?? id` 修的，OpenHands 是在資料模型裡就有這個概念** |
...
- **順帶印證 Lesson 26**：`conversation-state-event.ts:5-17` 的 `TokenUsage`
  ...
  **反推**出來的。別人在型別裡就寫明了。
- **也有一個要拿來當反面教材的**：`action-event.ts:63` 的
...
- 這一課的風險是寫成「介紹七種事件型別」。判準應該是下面這個實驗，
  而它會把 37 直接接回 29：
...
### Lesson 35 和 36 不是同一件事，README 要講清楚
...
| 專案 | 處置 | 理由 |
|---|---|---|
| **CrewAI** | 併進 **Lesson 19（subagent 委派） | 值得看的不是 Researcher / Writer / Reviewer 這種角色名字，是任務依賴怎麼表示、context 什麼時候該隔離、某個 agent 失敗整條 workflow 怎麼辦。要做的實驗是「一個 agent 做三步 vs 三個 agent 各做一步」，比成功率、token、資訊遺失、出錯後定不定得位。預期結論值得先寫下來再驗：多 agent 不會比較聰明，它是把狀態邊界變明確；沒有真的隔離需求時只是多付溝通成本** |
...
| **AutoGPT** | 當 **Lesson 24 的歷史對照，不寫 code | 「控制流交給模型」vs「控制流留在程式裡」，Lesson 24 已經用 deep-research 做過同一個對照，而且結論更硬（`breadth/2`、`depth-1` 寫死）。AutoGPT 的價值是它是這個錯誤最早、最出名的版本**，一句話帶過就夠 |
...
- **但有一塊很難割捨，記下來備查**（盤點時實際數的）：
  ...
  **「模型講了一段剛好以 `{` 開頭的話」和「模型要呼叫工具」，
  ...
  > 是它的位置在「換成本地模型」那一步，不在「從零學 agent」那條線上。
  > 真要寫，放 Lesson 50。
...
- **其餘（prefill/decode、continuous batching、PagedAttention、
  ...
  而那正好否決了「先寫一個 TypeScript 模擬器」的做法：
...
- **最小實驗**（本地假的收費 API，不需要碰任何真的錢或鏈）：
  ...
  「寫入成功、journal 寫入完成之前 crash」是**同一個窗口**，
  只是這次窗口的另一邊是錢。
...
- 這一課的問題必須是 agent 的問題，不是 TTS 的問題，
...
兩份建議在這一條上打架，處置是按範圍拆開，不是二選一，也不是各寫一次：
...
### ~~比上面四個缺口更前面的：Lesson 18-19 還在主線上~~ 2026-07-30 補完
...
## 2026-08-01 英文預設大掃除：重測之後有哪些數字變了

每一課的執行輸出、fixture 資料、README 範例輸出全部從中文換成英文，
然後每一段真模型的輸出都重新對 Gemini 3.6 Flash 跑過一次，而不是翻譯過去。
下面所有東西都是「重跑」才浮出來的；只翻譯的話一個都不會出現。

每一列都可以用它那一課 README 裡的指令重現。

### 確定性的部分，重測完全一樣

這一組是對照組：如果這次大掃除弄壞了結構性的東西，它們也會跟著動。

| 項目 | 結果 |
|---|---|
| Lesson 12 | 問「8/1」還是排到 `2024-08-01`，一模一樣 |
| Lesson 30 | `tuple` → 400、`multipleOf` → 安靜給 70；相容層兩個都修好，12/12 |
| Lesson 26 | Gemini 差額 94-1250，OpenAI 差額全部是 0 |
| Lesson 37 | detached 那一列**一模一樣**（11 / 7 / 0 / 0 / 0），36 次仍然沒有一次高估 |
| Lesson 7 | 7/7，而且連續兩次 `--compare` 每個案例都是 ±0 |

### 記錄下來但已經重現不了的發現

這些都不是程式的缺陷，是同一個模型五天後的樣子，也正是提議中的原則 10 的理由。

| 課 | 當初記錄 | 重測 | 狀態 |
|---|---|---|---|
| 8 / 29 | 被拒絕後 3/3 謊報 | 六次裡兩次（變更檔數 0/6） | README 已改寫；重點變成「三次謊一次比每次都謊更糟」 |
| 15 | 沒防禦時注入 3/3 成功 | **四次全部失敗** —— 這個載荷不再管用 | 2026-08-02 收掉；診斷是錯的——失效的是冒充，不是注入 |
| 19 | 委派貴 3.9 倍 | token 1.07 倍 / 呼叫 1.6 倍 | README 已改寫；改成掉但書那個失敗出現了，三次一次 |
| 25 | 真實報告有三個發現 | 0 條沒引用、0 個嫁接 | README 改成用「埋進去的」嫁接當例子 |
| 17 | `DEMOTE=off` 九次裡兩次是壞結果 | 3/3 都答對，但搜 4 次 vs 2 次 | 成本差重現得了，答錯率重現不了 |
| 18 | 六次裡三次被守衛擋下 | 三次都沒擋到——而那是守衛本來就該放行的題目 | 2026-08-02 收掉；缺口在詞序，而那個 0 對 recall 什麼都沒量到 |
| 1 | 找到埋的 bug 再加另外四個 | 埋的那個，有時候多一個 eviction | README 記錄了這個變異 |

### 2026-08-02 收掉

四個一起處理完，而且**其中三個的原始記錄都寫錯了自己的主題。**
這才是這一輪值得抽出來的模式：每一項當初都是照外觀分類的，不是重跑一次分類的，
而重跑之後每一次診斷都變了。

- [x] ~~**Lesson 15 的注入實驗沒有鑑別力了**~~ done 2026-08-02
      先重現了 0/4，然後發現**診斷是錯的**：失效的是那個*冒充*，不是那個注入。
      偽造 `[System note:]` 的載荷現在 6 次中 1 次；
      寫成**使用者自己的長期要求**的載荷，**關閉防禦 6 次中 5 次、
      打開防禦 6 次中 0 次**。兩個載荷都留在程式裡
      （`PAYLOAD=system-note|preference`）。換來的發現比原本那個好：

      > 記憶提供者的全部工作就是把使用者的偏好轉述回模型面前，
      > 所以一個偽造的偏好跟它**合法的輸出完全無法區分**——
      > 沒有偽造的標籤、沒有被冒充的說話者、沒有東西給過濾器抓。
      > 圍欄還是擋得住，因為**它不判斷內容，它標記通道。**

      另外寫下一個限制：污染警告是對偽造圍欄那個載荷發出的，
      對真正有效的那一個**不會**發出。圍欄擋得住它，紀錄卻一片安靜。
      `agent.ts` 現在會拒絕替「載荷根本沒進到 context」的那一次評分（原則 10，寫進程式）。

- [x] ~~**Lesson 18 的守衛有 recall 缺口**~~ done 2026-08-02
      確認了，而且**比記錄的更糟**：問題不是「散文 vs 指令」，是詞序。
      `agentd restart` 擋得下來，`restart agentd` 放行。
      那個 0/3 也不是量測——唯一跑過的題目是 *reload* 那一題，
      而在那一題上什麼都不擋才是對的答案。

      > **一個安全檢查只在「它本來就該放行」的輸入上量過，
      > 對它的 recall 什麼都沒說。** 在那個題目上，
      > 一個根本擋不了任何東西的守衛，分數跟完美的守衛一樣。

      加了 `TASK=restart`（一個真的需要換掉 daemon 的題目）、
      分支 E（散文，順序不拘），以及 `matchesCommandShapeOnly()`，
      讓那個落差變成量出來的一欄而不是一句宣稱。六次實測：
      **守衛擋了 5 個，舊比對器只抓得到 1 個**。
      `reload` 跟 `start` 刻意保持放行，而 `\bagentd\b` 必須收緊，
      因為它會 match 到 `/tmp/agentd-cache` 裡面——這一課自己那個無害的題目。
      新增 5 個測試。

- [x] ~~**Lesson 25 的檢查器剩下的全是假陽性**~~ done 2026-08-02
      連字號修正做了（`packWordBoundaries`），7 → 5。但**「七個全部是連字號和
      大小寫」這句話本身就是錯的**，而去 grep 語料才看得出來：只有兩個是連字號。
      `NVIDIA`、`GPU`、`third-party` **在整份語料裡都不存在**——
      正確的告警，那是模型自己的用字。`open-source` 在三個頁面上有，
      但那條 claim 引用的兩頁都沒有——也是正確的。
      剩下一個真的假陽性（`hardware-validated` vs "validated on hardware"，詞序沒對上），
      寫下來但不修，因為修它意味著要比對詞袋，而 `open-source` 會退化成兩個停用詞。

      > 七個發現裡有五個**看起來**有連字號，所以整份清單是照外觀分類的。
      > **假陽性率是量出來的，不是看出來的。**

      還有：根本原因是「有連字號就是識別字」這個啟發式，在散文是中文的時候很精準
      （中文的一般詞彙不會用連字號連起來），換成英文就變成假陽性製造機。
      **一個啟發式規則的可攜性，只到它當初被調校的那個語言為止。**
      `baseline.json` 從英文化大掃除之後就過期了（12 claims / 68 atoms），
      所以 `--compare` 一直在報假的退步；已重存。

- [x] ~~**Lesson 19 的成本論點要做一個決定**~~ done 2026-08-02
      決定：**以掉但書開場。** 先各重測 3 次，所以這個決定現在有證據撐著，
      不只是編輯判斷——
      ...
      重測時發現的新細節：掉了但書的那一次，**子 agent 自己的摘要裡但書還在**。
      遺失發生在父 agent 把三份摘要摺成一個答案的時候。
      **會失真的摘要有兩層，不是一層**，而失敗會在不同次之間於兩層之間移動。

### 這次順手修掉的

- [x] ~~**Lesson 24 的 token 上限是照中文調的**~~ done 2026-08-01
      clarify / queries / extraction 分別是 800 / 1200 / 2000。英文回覆比較長，
      而且在推理型模型上 thinking token 是從同一份額度扣的（Lesson 26 自己的
      主張），所以四個 query 裡有三個回傳零條結論、報告也被截斷。
      調成 1600 / 2500 / 4000 之後，證據從 3 條變成 14 條，而且沒有截斷。
- [x] ~~**欄寬是照全形字調的**~~ done 2026-08-01
      lesson-08 決策表和 lesson-17 / 26 / 30 輸出的 `padEnd` 都是照 CJK 手調的。
      只影響顯示，但在調寬之前每一張表都是歪的。
- [x] ~~**`COMPACTION_PREFIXES` 必須跟著壓縮包裝走**~~ done 2026-08-01
      `shared/search/index.ts` 比對的是 `shared/compaction.ts` 寫出來的那個
      字面前綴。翻譯了包裝卻沒改這裡的話，壓縮摘要會重新被搜出來——
      正是 Lesson 17 Step 3 存在要防的那個 bug。
- [x] ~~**Lesson 2 的 README 寫 7 個項目**~~ done 2026-08-01
      `list_files` 回傳 8 個。這是本來就有的漂移，跟這次大掃除無關。

### 刻意保留中文的地方

這些不是漏掉的。每一個都是「關於中文」的內容，翻掉就等於把被示範的東西刪掉。

| 位置 | 理由 |
|---|---|
| lesson-22 的 `q2-chinese` query、lesson-20 的搜尋範例 | 它們的用途就是示範 BM25 掛零、dense 救得回來 |
| `shared/search/index.ts`、`shared/memory/file-provider.ts` 裡的 CJK 分支 | 這是還在運作的斷詞支援，不是散文 |
| `tests/ai-search.test.ts` 的 fixture | 它們測的就是 CJK 斷詞和「英文月份 → 數字月份」的比對 |
| zh-TW README 的正文 | 那就是翻譯的意義 |
...
- [x] ~~**Lesson 2-5 沒有用真模型端到端測過**~~ 2026-07-27 完成
      ...
      - Lesson 4：session 寫入完成 12 筆、`--resume` 正常、`/tree` 正常
...
- [x] ~~**Lesson 8-9 沒有接進真的 agent**~~ done 2026-08-03
      從後續那一輪開始就過期了；這一格跟同一份檔案裡的「Lesson 8 後續」、
      「Lesson 9 後續」兩節互相矛盾，那兩節描述的正是
      `lesson-08-permissions/agent.ts` 加 `fake-provider.ts`、
      以及 `lesson-09-unattended/agent.ts` 加 `fake-provider.ts` 加
      `email-tool.ts`，而且都有真模型實測。只是這個框沒有被打勾。

      > 這件事值得寫下來而不是默默修掉：**一份長到會跟自己矛盾的文件，
      > 要用跟課程一樣的方式對待**——2026-08-02 那一輪靠重跑找出三份
      > 寫錯自己主題的記錄，而這一格是對一個目錄列表寫錯了。
...
- [x] ~~Lesson 6-7 其實不能用 `PROVIDER=fake` 跑~~ 2026-07-27 修好
      ...
      > 但要注意：`PROVIDER=fake bun run lesson-07` **只驗證評估管線本身**
...
- [x] ~~**只測過 Gemini**~~ → **OpenAI 已實測**（2026-07-27）
      ...
      Gemini     94 ~ 1250     ← thinking 不在 completion_tokens 裡
...
- [x] ~~Lesson 1-5 的 playground 是同一份複製五次~~ 2026-07-28（部分）
      **只改了 Lesson 5，而且是有理由的**：那一課的 playground 太小
      ...
      而且第二個 bug 埋在 `analytics.ts`，要跨檔案才找得到。
