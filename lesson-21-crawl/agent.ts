 ...
 *
 * One difference from Lesson 20: **fetch_page was added.**
 *
 *   Lesson 20   web_search                → only snippets, so it can only guess
 *   Lesson 21   web_search + fetch_page   → it can open the page and look
 *
 * `runTurn` still has not changed a line (since Lesson 3).
 * `web_search` is imported straight from Lesson 20 rather than copied —
 * what this lesson proves is what one added tool changes,
 * so everything except that tool has to be identical.
 *
 * Run:
 *   bun run lesson-21                     with a real model
 *   PROVIDER=fake bun run lesson-21       no key needed
 */

import { resolve } from "node:path";
import { webSearchTool } from "../lesson-20-search-agent/tools/search.ts";
...
import { fakeCrawlProvider } from "./fake-provider.ts";
import { fetchPageTool } from "./tools/fetch.ts";
...
## Your tools
- web_search: ranked results with title, URL, date and a short snippet.
...
// ─────────────────────────────────────────────────────────────
// The agent loop: identical to Lessons 3 / 6 / 20, not one line changed
// ─────────────────────────────────────────────────────────────
...
function selectProvider(): StreamingProvider {
	if (process.env.PROVIDER?.toLowerCase() === "fake") return fakeCrawlProvider();
