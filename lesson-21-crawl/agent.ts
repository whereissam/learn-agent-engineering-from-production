 ...
 *
 * 跟 Lesson 20 的差別只有一個：**多了 fetch_page。**
 *
 *   Lesson 20   web_search                → 只看得到 snippet，只能猜
 *   Lesson 21   web_search + fetch_page   → 可以把頁面打開來看
 *
 * `runTurn` 還是一行都沒改（從 Lesson 3 到現在）。
 * `web_search` 直接 import Lesson 20 的，不複製一份——
 * 這一課要證明的就是「加一個工具能改變什麼」，
 * 所以除了那個工具以外，其他東西都必須一模一樣。
 *
 * 執行：
 *   bun run lesson-21                     用真模型
 *   PROVIDER=fake bun run lesson-21       不需要金鑰
 */
...
// ─────────────────────────────────────────────────────────────
// Agent loop: 跟 Lesson 3 / 6 / 20 完全相同，一行都沒改
// ─────────────────────────────────────────────────────────────
