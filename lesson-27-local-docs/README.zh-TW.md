...
> [English](README.md)
>
> 前置：[Lesson 21](../lesson-21-crawl/README.zh-TW.md)（chunk）、[Lesson 22](../lesson-22-retrieval/README.zh-TW.md)（RRF 與排序）。
>
...
4. 一邊完全不相關的時候會發生什麼事？（這題的答案改了三次才對）

語料就是**這個 repo 自己的 markdown**：22 個檔案、214 個 chunk。
不用另外準備資料，而且每一筆結果都讀得懂。
...
> 而那 4 個位置本來可以放這個 repo 自己寫過 chunk 的段落。
...
| RRF | 本系列 [Lesson 22](../lesson-22-retrieval/README.zh-TW.md) |

> gpt-researcher 支援本地文件的方式是「把它們當成另一個 retriever」，
> 跟這一課的做法形狀一樣。差別是它把本地文件也做了 embedding，
> 這裡只做 BM25——**因為這些文件會一直改，而它們的使用情境是
...
[docs/TODO.md](../docs/TODO.zh-TW.md)：Lesson 28-37 處理的是
