...
> **Hermes 篇最後一課。** 前置：[Lesson 5](../lesson-05-compaction/)（context 壓縮）、
> [Lesson 9](../lesson-09-unattended/)（批准）、
> [Lesson 18](../lesson-18-scheduling/)（無人值守）。
>
...
> 它也直接接回 [Lesson 18](../lesson-18-scheduling/)：那一課的守衛擋的是
...
> **摘要不是證據。** 這是 [Lesson 29](../lesson-29-evidence/) 的結論在
...
> 而且代價比我預期的大 —— 我猜 1.5-2 倍，實際是 **3.9 倍**。
>
> ❌ **錯的那一半**：我預期那個但書會在摘要那一層掉。**沒有掉，3/3 都活著。**
> 負責 inventory 的那個子 agent 每次都把它寫進摘要了。

第二點值得記下來，因為它是一個**反方向的教訓**：

> 我設計了一個專門用來製造資訊遺失的機關（把但書放在檔頭），
> 然後模型每一次都沒有掉。**這不代表委派不會掉資訊**，
...
| 遞迴委派的正確做法 | 我們直接禁止。**要允許的話，你需要的是深度預算 + 全域 token 預算，那是 Lesson 24 的形狀** |
...
> [Lesson 17](../lesson-17-search/) 的跨 session 搜尋結果裡。
...
**照閱讀順序**，接下來是證據那條支線：
[Lesson 29](../lesson-29-evidence/) → 28 → 37。
