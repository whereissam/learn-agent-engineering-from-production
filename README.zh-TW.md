...
**下面 29 步都寫完了、都跑得起來。** 第 1-17 步是核心；第 18-25 步是
一條**可以整包跳過**的領域支線，第 26-29 步回到 loop 周圍的 harness。
規劃中的課接在第 29 步後面。
每一步都標出你在讀哪個專案的哪一段。
...
flowchart LR
    ...
    P5 --> P6["第 26-29 步<br/><b>loop 周圍那一圈</b><br/>Mastra、OpenCode、OpenHands"]
```
...
**「證據」這條線完整了**（29 → 28 → 37：量測、生命週期、型別，
三課講的是同一句話 —— **紀錄不能比事實更樂觀**）。
**現在最想要的是 Lesson 33**：把 loop 變成可以序列化的狀態機，
那是 suspend / resume 的前提，也是所有「durable agent」說法真正靠的東西。

### Prod 篇 · Lesson 50-59 —— **不算在 29 步裡**
...
- **機制**（權限、inbox、排序、引用檢查、檢索）由 **189 個確定性檢查覆蓋：188 通過，1 個是刻意 skip 的**
...
- **Lesson 37** — 同一個動作，模型當審查員時 3/3 評 HIGH，
  自己要動手時評 LOW。36 次評估裡**沒有一次**評得比 harness 高。
  模型自評風險可以當訊號，不能當閘門。
