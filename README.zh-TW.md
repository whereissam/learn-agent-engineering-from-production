...
- **十二個真實專案被 clone 進這棵樹、而且讀到行號**，不是做摘要。
  `bun run check:citations` 會把每一課裡每一個 `file.ts:123` 對它指名的來源解析
  一次，只要有一個指向不存在的行就讓它失敗
- **每一個機制都會被關掉、然後量測。** 一課要站得住，前提是把它的機制關掉會產生
  一個你看得見的失敗。「200 個工具」不是一個關於 context window 的意見；它是
  OpenAI 回的一個 `400`
- **當量測結果跟計畫牴觸時，課裡會講出來。** 這裡有好幾課推翻了
  [docs/TODO.md](docs/TODO.md) 原本的預測，而那些更正被留在看得見的地方，
  沒有被收拾掉

那就是它跟「從零打造一個 agent」的差別：從零教你那個 loop。這裡教你的是，當其他人
開始依賴它之後，那個 loop 會長成什麼——而它是從已經扛著那些人的程式碼裡學來的。
...
> | 14 audit log | **刪掉**，之後號碼被重新使用。[Lesson 8](lesson-08-permissions/) 的練習 4 是二十行的 audit log；[第 14 課](lesson-14-tracing/)是它答不出來的那一半——這是誰的請求付的錢、花了多少 |
>
