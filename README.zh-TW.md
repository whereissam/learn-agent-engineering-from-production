...
**課程實作裡不用任何 agent framework**——每個機制都自己重建一次，
而不是把它藏在框架後面。

**大部分的課都是先讀一個真實開源專案的原始碼**，再抽成最小的可跑版本。
例外是那幾課——**開源專案在那裡露出來的是一個缺口，不是一個解法**：
領域工具、評估、引用驗證都是從那個缺口長出來的，而且課裡就寫明了。

## 一句話講完

> Agent 就是一個 while 迴圈：問模型 → 它說它想用某個工具 →
> **你的程式**去執行那個工具 → 把結果餵回去 → 直到它不再要求用工具。
...
最違反直覺的一點：**模型什麼都不能做。** 它不能讀檔、不能上網、不能執行指令，
唯一能做的是輸出文字。所謂「呼叫工具」，只是一段結構化的文字，
請**你的程式**去做事。

這個迴圈大概 50 行。剩下的全部——權限、session、壓縮、沙箱、
「怎麼證明事情真的做了」——都是它周圍的工程。**這個系列做的就是把那些工程一個一個重建，
而且每一個都親手弄壞一次。**
...
| **2. Harness** — 權限、server、schema、執行證據、durability、沙箱 | Lesson 8-12、30 已寫；28-29、31-37 規劃中 |
| **3. 長期運行** — 記憶、skills、排程、委派 | Lesson 15-17 已寫；18-19 規劃中 |
| **4. 領域工具** — 能力上限取決於它能操作什麼，不是 prompt 多漂亮 | Lesson 6；Lesson 20-27 是大型示範 |
| **5. Evaluation** — 沒有它就不知道改動到底有沒有變好 | Lesson 7、22、25 |

第 4、5 層才是護城河，所以這系列**不會叫你「自己想辦法」**：
Lesson 6 從零設計一整組領域工具，Lesson 7 跑完
「量測 → 發現問題 → 修 → 確認沒退步」的閉環。
...
**下面 28 步都寫完了、都跑得起來。** 第 1-17 步是核心；第 18-25 步是
一條**可以整包跳過**的領域支線，第 26-28 步回到 loop 周圍的 harness。
規劃中的課接在第 28 步後面。
每一步都標出你在讀哪個專案的哪一段。
...
flowchart LR
    ...
    P5 --> P6["第 26-28 步<br/><b>loop 周圍那一圈</b><br/>Mastra、OpenCode"]
```
...
| | | *29 排在 28 前面是刻意的：它直接回答第 8 步留下的問題，而 28 是同一個問題更難的版本。* | |

第 18-25 步可以整包跳過，它是第 6 步那套方法的大型示範，不是任何東西的前置。
每一課的 README 開頭都會再標一次自己的前置。

### 規劃中的續篇 · Lesson 32-37 —— 還沒寫

它們接在第 28 步後面，而且**這張表裡的課目前都跑不起來**。
...
| 37 | agent 動作了——action 和 observation 怎麼變成 history？ | OpenHands |

這張表刻意很短。一個專案要換到**主線**的一課，必須有真正的 agent loop 或
workflow、碰到 tools / context / memory / permission / session 之一，
而且有一個**關得掉、關掉就看得到失敗**的機制。
**一個只會對爛專案說不的判準沒有用，這一個會對好專案說不。**

**現在最想要的是 Lesson 37。** 「證據」這條線已經有兩課：
29 量測真正變了什麼，28 讓被中斷的那一輪不會留下騙人的紀錄。
37 是第三個角度 —— **把「誰說的」寫進型別**，
讓「模型說的」和「環境說的」永遠不可能擠在同一個欄位裡。

### Prod 篇 · Lesson 50-59 —— **不算在 28 步裡**
...
vLLM 和 Fish Speech 都 clone 下來盤點過才放進這一篇的，
行數、入口檔案、以及一個授權上的坑都記在 [docs/TODO.md](docs/TODO.md)。
...
bun run test                        # 179 通過、1 skip；不用 API key
```

`fake` 是照腳本回應的假模型。它不會思考，但**整個 loop 是完全真實的**：
真的呼叫工具、真的讀檔案、真的處理錯誤。
配 debugger 單步走一遍，是理解 loop 最快的方法。

要用真模型的話，把**一把** key 放進 `.env`（`cp .env.example .env`）：
...
有一半的課**跑起來完全不會呼叫模型**。這不是還沒做完，
而是因為它們教的東西不在模型裡。

> **模型是這個系列裡唯一一個你不用蓋的零件。**

- **機制**（權限、inbox、排序、引用檢查、檢索）由 **180 個確定性檢查覆蓋：179 通過，1 個是刻意 skip 的**
  （那是要真 provider 才跑的契約測試，因為它會花錢）。
- **模型行為**分開量，用真的 Gemini 3.6 Flash 反覆跑，每一次都記在
  [docs/TODO.md](docs/TODO.md) 裡，**包括結果跟我預期相反的那幾次**
  （Lesson 17、27）和**結果很難看的那次**（Lesson 8）。

> 如果一個機制的正確性要靠模型才能驗證，那它就不是機制，是祈禱。

**還沒驗證的**：Anthropic 的 provider 沒跑過真模型（沒有 key）。
OpenAI 跑過，而且量到一個跨 provider 的差異：同樣是
`total - input - output`，Gemini 的差額是 93~1353（thinking 不算在 output 裡），
...
- **Lesson 28** — 一個被真模型抓出來的收尾 bug，我自己設計的六格矩陣沒抓到：
  ...
  **成功的**工具被記成 interrupted，而訊息的 finish 是 "end"。
  **腳本化的實驗只覆蓋你想得到的路徑。**
...
**沒有逐行讀過**，只用來標定位——Lesson 22 在課裡就寫明了這一點，
不會假裝欠了一筆沒有欠的債：
...
這個 repo 沒有整包複製任何一個上述專案。每一課只隔離**一個**機制、
重建一個最小可跑版本，然後連回啟發它的那個上游檔案。

**每個上游專案有自己的授權，而且不是每一個都寬鬆。**
Fish Speech 用的是 Fish Audio Research License，不是 MIT 或 Apache——
**這件事是 clone 下來才發現的，光看 README 看不出來。**
要重用任何程式碼或模型權重之前，先確認上游授權。
