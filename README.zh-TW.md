...
下面 35 步都寫完了、都跑得起來。第 1-19 步是核心；第 20-27 步是一條可以整包
跳過的領域支線，第 28-35 步回到 loop 周圍的 harness。規劃中的課接在第 35 步
後面。每一步都標出你在讀哪個專案的哪一段。

```mermaid
flowchart LR
    P1["第 1-5 步<br/><b>引擎本體</b><br/>Pi"] --> P2["第 6-7 步<br/><b>你的領域</b><br/>自己做"]
    P2 --> P3["第 8-14 步<br/><b>從 loop 到能用的系統</b><br/>OpenWorker + Mastra"]
    P3 --> P4["第 15-19 步<br/><b>跑好幾個月</b><br/>Hermes"]
    P4 --> P5["第 20-27 步<br/><b>一整個領域</b><br/>四個搜尋專案"]
    P5 --> P6["第 28-35 步<br/><b>loop 周圍那一圈</b><br/>Mastra、OpenCode、OpenHands"]
```

> **課號會跳，步驟號不會跳。** 只有 14 還空著。11 與 13 的內容被消化進別課，
> 而它們的號碼後來被重新用在該待在那個位置的題目上：
>
> | 課號 | 它去了哪裡 |
> |---|---|
> | 11 connector 與 OAuth | **拆開**，之後號碼被重新使用。OAuth **協定**那一半是 [Lesson 12](lesson-12-mcp/) 裡的「OAuth 半課」；[第 11 課](lesson-11-credentials/)現在是憑證的**生命週期**——過期、擁有者、撤銷。把 25 個 connector 收成一層抽象仍然延後到 Prod 55 |
...
| 11 | [11 憑證](lesson-11-credentials/) | token 半夜三點過期了，它會怎麼做、那又是誰的 token？ | Mastra `mcp/client/oauth-provider.ts:368` |
| 12 | [12 MCP client](lesson-12-mcp/) | 接別人寫的工具，怎麼不被拖垮？ | OpenWorker `mcp/`（647 行） |
| 13 | [13 工具漂移](lesson-13-tool-drift/) | server 在你批准之後把工具改掉了，怎麼辦？ | Mastra `mcp/client/client.ts:286` |
| 14 | [30 Schema 相容](lesson-30-schema-compat/) | 別人的 schema 你改不了，那會壞在哪？ | Mastra `schema-compat/` |
...
| 15 | [15 長期記憶](lesson-15-memory/) | 這次學到的，下次怎麼還記得？ | Hermes `memory_manager.py` |
| 16 | [16 Skills](lesson-16-skills/) | 能力怎麼累積，又不弄髒 context？ | Hermes `skill_utils.py` |
| 17 | [17 跨 session 搜尋](lesson-17-search/) | 上個月那個 session 怎麼找回來？ | Hermes `session_search_tool.py` |
| 18 | [18 排程與無人值守](lesson-18-scheduling/) | 半夜三點自己跑，跑到一半死掉怎麼辦？ | Hermes `cron/`（8,727 行） |
| 19 | [19 委派](lesson-19-delegation/) | 把任務交給子 agent，它看得到什麼？ | Hermes `delegate_tool.py` |
| | | **⑤ 一整個領域 · Lesson 20-27 · 四個搜尋專案**<br/>可以跳過，但它才是真實的樣子。 | |
| 20 | [20 最小的 search agent](lesson-20-search-agent/) | 模型怎麼看到訓練資料以外的東西？ | deep-research |
| 21 | [21 Crawl 與內容抽取](lesson-21-crawl/) | 搜尋結果點進去之後呢？ | Crawl4AI、Firecrawl |
| 22 | [22 檢索與排序](lesson-22-retrieval/) | 找到一堆結果，哪些真的相關？ | txtai |
| 23 | [23 對照真實原始碼](lesson-23-real-world/) | 真實產品跟我們的玩具差在哪？ | 四個專案逐行對照 |
| 24 | [24 Deep Research loop](lesson-24-research-loop/) | 研究幾十個網頁，控制流誰說了算？ | `deep-research.ts:230` |
| 25 | [25 引用與評估](lesson-25-citations/) | 報告裡的引用是真的嗎？ | 沒有人，四個專案都不驗 |
| 26 | [26 成本與預算](lesson-26-cost/) | 錢到底花在哪一步？ | gpt-researcher `costs.py:63` |
| 27 | [27 本地文件 + web](lesson-27-local-docs/) | 自己的文件跟 web 怎麼混在一起搜？ | gpt-researcher `document/` |
| | | **⑥ loop 周圍那一圈 · Mastra、OpenCode、OpenHands** | |
| 28 | [31 Processor pipeline](lesson-31-processors/) | guardrail 怎麼留在 loop 外，而且 secret 不漏進任何 sink？ | Mastra `core/src/processors/` |
| 29 | [32 工具搜尋](lesson-32-tool-search/) | 接上十個 MCP server，200 個工具塞得進去嗎？ | Mastra `processors/tool-search.ts` |
| 30 | [33 可續跑的 run](lesson-33-durable/) | process 半夜三點死了，那個 run 在哪裡？ | Mastra `workflows/state-reader.ts` |
| 31 | [29 完成的證據](lesson-29-evidence/) | 模型說「改好了」，憑什麼相信它？ | OpenCode `snapshot/index.ts` |
| 32 | [28 中斷之後的一致性](lesson-28-consistency/) | 跑到一半被殺掉，存下來的 session 還能相信嗎？ | OpenCode `session/processor.ts` |
| 33 | [37 Action / observation](lesson-37-trajectory/) | agent 宣稱的跟環境量到的，可以放在同一個欄位嗎？ | OpenHands `core/events/` |
| 34 | [35 權限引擎不是沙箱](lesson-35-sandbox/) | 指令被准了，它現在碰得到什麼？ | Anthropic SRT `macos-sandbox-utils.ts` |
| 35 | [38 你自己弄壞的那個 cache](lesson-38-prompt-cache/) | 你 prompt 裡的哪一行，讓你賠掉整個前綴？ | OpenCode `protocols/utils/cache.ts` |
| | | *29 排在 28 前面是刻意的：它直接回答第 8 步留下的問題，28 是同一個問題更難的版本，而 37 把答案寫進型別。Lesson 35 回答第 8 步的另一半，需要 macOS。* | |

第 20-27 步可以整包跳過，它是第 6 步那套方法的大型示範，不是任何東西的前置。
每一課的 README 開頭都會再標一次自己的前置。

### 規劃中的續篇 · Lesson 34 與 36，還沒寫

它們接在第 35 步後面，而且這張表裡的課目前都跑不起來。主要來源都 clone
