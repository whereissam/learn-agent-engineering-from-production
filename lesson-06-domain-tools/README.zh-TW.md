...
> [English](README.md)
>
> 前置：[Lesson 3](../lesson-03-streaming/README.zh-TW.md)（streaming）。
>
...
後兩個是後來補的。原本五個有一個共同的形狀（一個 session 一個突發事件），
而真實 telemetry 不長那樣。`sess_007` 特別值得看：
...
這也回答了一個常見問題：「要做一個 XX 領域的 agent，該用哪個 framework？」
...
在 system prompt 裡寫「請附上證據」，模型大部分時候會照做。
在工具裡檢查，它每次都得照做，否則工具就失敗，它得重來。
...
streaming provider 在重建訊息時把 `extra_content` 丟掉了，於是 400。
...
[Lesson 7: Evaluation](../lesson-07-evaluation/README.zh-TW.md)：你現在有一個會分析事故的 agent。
但它準不準？換個 model 會變好還變壞？改了 prompt 有沒有退步？
