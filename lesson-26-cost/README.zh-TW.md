...
> [English](README.md)
>
> 前置：[Lesson 24](../lesson-24-research-loop/README.zh-TW.md)（結構性預算）。
>
...
當時的判斷是「13 條證據太多、3000 token 寫不完」，
真正的原因是**thinking 先吃掉了大部分額度**。
...
這一課選後者，然後用一個 `classify(request)` 函式從 prompt 認出步驟。
...
| 結構性預算（次數上界） | 本系列 [Lesson 24](../lesson-24-research-loop/README.zh-TW.md) 的 `estimateCost()` |

> gpt-researcher 用的是**估算**（照字元數估 token），這一課用的是
...
[Lesson 27: 本地文件 + web 混合檢索](../lesson-27-local-docs/README.zh-TW.md)：把 Lesson 22 的檢索
接到自己的檔案上。
