...
> [English](README.md)
>
> 前置：[Lesson 20](../lesson-20-search-agent/README.zh-TW.md)、[21](../lesson-21-crawl/README.zh-TW.md)、[22](../lesson-22-retrieval/README.zh-TW.md)。
>
...
`.gitignore` 是給**所有讀者**的檔案，這幾份只是本機的參考資料，
...
> 順帶一提，這個檢查器第一次跑就抓到這一課自己引用裡的三條 off-by-one。
> 上面那個 `~` 就是它的樣子。
...
Lesson 20 Step 4 觀察到：模型會生出 `site:github.com`、`OR`、引號，
而這些在 BM25 眼裡只是普通的字。當時寫的結論是「這不是模型的錯，是 harness 沒做」。
...
[Lesson 24: Deep Research loop](../lesson-24-research-loop/README.zh-TW.md)：Step 4 讀到的四個機制，
自己實作一次。
